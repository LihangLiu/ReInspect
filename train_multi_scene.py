...
def load_multi_scene_imglist_txt(txt_file):
    res = []
    dir_path = os.path.dirname(os.path.realpath(txt_file))
    for line in open(txt_file, 'r').readlines():
        sub_txt_path = os.path.join(dir_path, line.strip())
        assert (os.path.exists(sub_txt_path) and sub_txt_path[-4:]=='.txt'), "%s do no exists"%sub_txt_path
        res.append(load_imname_list(sub_txt_path))
    return res
...
def load_imname_list(imfile):
    ...
    imname_list = [os.path.join(os.path.dirname(os.path.realpath(imfile)), line.strip()) 
                    for line in lines]
    print 'load', os.path.split(imfile)[-1], len(imname_list),'images'
    return imname_list
...
def train(config):
    ...
    boost_test_gen = load_idl(data_config["boost_test_gen"],
                                   image_mean, net_config, jitter=False)
    multi_scene_imglist = load_multi_scene_imglist_txt(data_config['multi_scene_imglist_txt'])  # [ [],[],... ]
    boost_imname_list = []
    for sub in multi_scene_imglist[-3:]:
        boost_imname_list += sub
    tmp = len(boost_imname_list)
    for sub in multi_scene_imglist[:-3]:
        boost_imname_list += sub[:tmp/(len(multi_scene_imglist)-3)]
    ...
    for i in range(solver["start_iter"], solver["max_iter"]):
        ...
        # # train
        learning_rate = (solver["base_lr"] * (solver["gamma"])**(i // solver["stepsize"]))
        # train on reinspect dataset
        re_net.phase = "train"
        re_net.copy_params_from(boost_net)
        for _ in range(10):
            forward(re_net, re_train_gen.next(), net_config, lstm_input_mean=lstm_input_mean_gen.next())
        ...
        forward(boost_net, boot_train_gen.next(), net_config, lstm_input_mean=lstm_input_mean_gen.next())
